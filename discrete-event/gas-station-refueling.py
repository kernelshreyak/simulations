...
CAR_TANK_SIZE = 100         # Size of car fuel tanks (liters)
CAR_TANK_LEVEL = [5, 25]   # Min/max levels of car fuel tanks (liters)
REFUELING_SPEED = 10        # Rate of refuelling car fuel tank (liters / second)
TANK_TRUCK_TIME = 60      # Time it takes tank truck to arrive (seconds)
T_INTER = [20, 60]        # Interval between car arrivals [min, max] (seconds)
SIM_TIME = 2500            # Simulation time (seconds)
# fmt: on
...
def gas_station_control(env, station_tank):
    ...
    while True:
        if station_tank.level / station_tank.capacity * 100 < THRESHOLD:
            ...
            print(
                f'{env.now:6.1f} s: *********************************************** Calling tank truck')
...
gas_station = simpy.Resource(env, 10)
